...
static uint8_t advertData[] = {
    ...
    0x03, // length of this data (1 byte for UUID, 1 byte for battery level)
    GAP_ADTYPE_SERVICE_DATA, // тип данных для сервиса
    LO_UINT16(BATTERY_SERVICE_UUID), // UUID сервиса батареи (например, 0x180F)
    0x50 // уровень заряда батареи (константа, например, 80%)
};
...
void Peripheral_Init()
{
    ...
    {
        uint8_t charValue1[SIMPLEPROFILE_CHAR1_LEN] = {8};
...
static void simpleProfileChangeCB(uint8_t paramID, uint8_t *pValue, uint16_t len)
{
    switch(paramID)
    {
        ...
        {
            uint8_t test_data[] = {0x22, 0x59, 0xd8};
            uint8_t newValue[SIMPLEPROFILE_CHAR1_LEN];
           
            tmos_memcpy(newValue, pValue, len);
            newValueFromBLE[SIMPLEPROFILE_CHAR1_LEN]=newValue;
           
            PRINT("--------------profile ChangeCB CHAR1..------------------- \r\n");
            PRINT("CHAR1 newValue [1] >>> %x \r\n",*(newValue+1));
            PRINT("CHAR1 newValue [2] >>> %x \r\n",*(newValue+2));

            PRINT("CHAR1 newValue>>> %x \r\n",*newValue);
            PRINT("CHAR1 pValue >>> %x \r\n",*pValue);
            PRINT("CHAR1 newValue pointer>>> %x \r\n",newValue,3);
           
            Send_LED_Message(newValue,3);
